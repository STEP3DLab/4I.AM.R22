...
const isSSR = Boolean(globalThis.IS_SSR);
const hasPrerenderedMarkup =
  typeof document !== 'undefined' && document.documentElement?.dataset?.prerendered === 'true';
function shouldKeepExistingContent(root) {
  return !isSSR && hasPrerenderedMarkup && root && root.childElementCount > 0;
}
...
function renderBenefits() {
  ...
  if (shouldKeepExistingContent(root)) return;
  root.innerHTML = '';
